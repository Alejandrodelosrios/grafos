 ...
 * @author EQUIPO
 */
public class OrdenamientoTopologicoPesado {
 private final DiGrafoPesado elDigrafo;//copia del digrafo
 ...
 private final ConexoPesado esConexo; //para ver si es debilmente conexo
 
 //constructor para grafos no pesados
 public OrdenamientoTopologicoPesado(DiGrafoPesado unDiGrafo){
  ...
  this.esConexo=new ConexoPesado(this.elDigrafo);//instancia una clase que ve si es conexo
 ...
 public void ejecutarOrdenamientoTopologico(){
  ...
  if(!this.ciclo.haycicloEnDiGrafo()&& esConexo.esDebilmenteConexo()){
   ...
   //busca la primera posicion que tenga grado 0
   int verticeEnTurno=this.gradosDeEntradas.indexOf(0);
   //lo mete a la cola y lo marca 
   ...
   do{
    ...
    verticeEnTurno=colaDeVertices.poll();//sacamos la posicion del vertice con grado 0
   ...
   Iterable<Integer> adyacentesDelVertice=this.elDigrafo.adyacentesDelVertice(verticeEnTurno);
    ...
    for(Integer adyacentes:adyacentesDelVertice){
     ...
     if(this.gradosDeEntradas.get(adyacentes)>0){   
 ...
 // devuelve el resultadodel ordenamiento topologico en forma de iterable
